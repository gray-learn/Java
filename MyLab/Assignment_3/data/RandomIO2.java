...
public class RandomIO2 {
	...
	public static Person findPersonByRecordNumber(String recordNumber, String filename) throws IOException {
		...
		try (RandomAccessFile file = new RandomAccessFile(filename, "r")) {
			...
			char[] d = recordNumber.toCharArray();

//			for (int z = 0; z < file.length(); z++) {
//				file.seek(z);
////				int recordNum = Byte.valueOf(file.readByte()).intValue();
//				System.out.println("Position==" + z + "file.readByteto char()==" + (char) file.readByte());
//			}
			...
			for (int i = 1; i <= count; i++) {
				...
				if (position >= 0) {
					...
					if (recordNumber.equals(recordNum)) {
						...
						System.out.println("recordId====" + recordNumber.getBytes());
